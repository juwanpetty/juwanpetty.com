...
function PostItem({ title, date, slug }: PostItemProps) {
  return (
    <Link
      ...
      href={`/writing/${slug}`}
    >
      <p className="text-sand-11 m-0 truncate transition-colors group-hover:text-red-500">
        {title}
      </p>
    ...
    <MainLayout>
      <div className="mx-auto w-full max-w-screen-sm px-4 pb-32 pt-16 sm:pt-32">
        <header className="space-y-2">
          <h1 className="text-sand-11 m-0 text-3xl font-semibold">Writing</h1>
        ...
        {groupedPosts.map((group) => (
          <div key={group.year} className="space-y-4">
            <div className="text-sand-11 mt-8 flex items-center justify-start font-medium">
